...
describe('Checkout', () => {
  ...
  describe('request handling', () => {
    ...
    it('should create checkout with single product', async () => {
      ...
      const checkout = Checkout({ accessToken: 'test-token' })
      const request = new NextRequest('https://example.com/checkout?products=prod_123')
    ...
    it('should create checkout with multiple products', async () => {
      ...
      const checkout = Checkout({ accessToken: 'test-token' })
      const request = new NextRequest('https://example.com/checkout?products=prod_123&products=prod_456')
    ...
    it('should add theme parameter to redirect URL', async () => {
      ...
      expect(response.headers.get('location')).toBe('https://polar.sh/checkout/123?theme=dark')
    })
    ...
    it('should parse customer parameters correctly', async () => {
      ...
      const checkout = Checkout({ accessToken: 'test-token' })
      const billingAddress = JSON.stringify({ street: '123 Main St', city: 'NYC' })
      ...
      const request = new NextRequest(
        ...
        `products=prod_123&` +
        `customerId=cust_123&` +
        `customerExternalId=ext_123&` +
        `customerEmail=test@example.com&` +
        `customerName=John Doe&` +
        `customerBillingAddress=${encodeURIComponent(billingAddress)}&` +
        `customerTaxId=TAX123&` +
        `customerIpAddress=192.168.1.1&` +
        `customerMetadata=${encodeURIComponent(customerMetadata)}&` +
    ...
    it('should handle allowDiscountCodes as false', async () => {
      ...
      const checkout = Checkout({ accessToken: 'test-token' })
      const request = new NextRequest('https://example.com/checkout?products=prod_123&allowDiscountCodes=false')
