...
const { handleWebhookPayload } = await vi.importMock('@polar-sh/adapter-utils') as any
const { validateEvent } = await vi.importMock('@polar-sh/sdk/webhooks.js') as any
const { APIError, createAuthEndpoint } = await vi.importMock('better-auth/api') as any
...
describe('webhooks plugin', () => {
	...
	describe('webhook endpoint handler', () => {
		...
		beforeEach(() => {
			...
			mockRequest = {
				headers,
				text: vi.fn().mockResolvedValue('{"type": "checkout.created", "data": {}}'),
		...
		it('should throw error when webhook secret is missing', async () => {
			...
			await expect(noSecretHandler(ctx)).rejects.toThrow('Polar webhook secret not found')
		})
		...
		it('should handle invalid webhook signature', async () => {
			...
			await expect(handler(ctx)).rejects.toThrow('Webhook Error: Invalid signature')
		...
		it('should handle missing webhook headers', async () => {
			...
			await expect(handler(ctx)).rejects.toThrow('Webhook Error: Missing required headers')
		})
		...
		it('should handle webhook payload processing errors', async () => {
			...
			await expect(handler(ctx)).rejects.toThrow('Webhook error: See server logs for more information.')
		...
		it('should handle non-Error validation failures', async () => {
			...
			await expect(handler(ctx)).rejects.toThrow('Webhook Error: Unknown validation error')
		})
