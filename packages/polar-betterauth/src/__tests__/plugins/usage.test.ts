...
const { APIError, createAuthEndpoint, sessionMiddleware } = await vi.importMock('better-auth/api') as any
...
describe('usage plugin', () => {
	...
	describe('meters endpoint', () => {
		...
		it('should list customer meters with pagination', async () => {
			...
			vi.mocked(mockClient.customerPortal.customerMeters.list).mockResolvedValue(mockMeters)
