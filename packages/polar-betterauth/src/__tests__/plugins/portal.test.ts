...
vi.mock('better-auth/plugins', () => ({
	createAuthEndpoint: vi.fn((path, config, handler) => ({ path, config, handler })),
}))

const { APIError, sessionMiddleware } = await vi.importMock('better-auth/api') as any
const { createAuthEndpoint } = await vi.importMock('better-auth/plugins') as any
...
describe('portal plugin', () => {
	...
	describe('portal endpoint', () => {
		...
		it('should create customer portal session and return URL', async () => {
			...
			vi.mocked(mockClient.customerSessions.create).mockResolvedValue(mockSession)
		...
		it('should handle API errors', async () => {
			...
			await expect(handler(ctx)).rejects.toThrow('Customer portal creation failed')
	...
	describe('state endpoint', () => {
		...
		it('should get customer state', async () => {
			...
			vi.mocked(mockClient.customers.getStateExternal).mockResolvedValue(mockState)
	...
	describe('benefits endpoint', () => {
		...
		it('should list customer benefits with pagination', async () => {
			...
			vi.mocked(mockClient.customerSessions.create).mockResolvedValue(mockSession)
			vi.mocked(mockClient.customerPortal.benefitGrants.list).mockResolvedValue(mockBenefits)
	...
	describe('subscriptions endpoint', () => {
		...
		it('should list subscriptions via customer portal', async () => {
			...
			vi.mocked(mockClient.customerSessions.create).mockResolvedValue(mockSession)
			vi.mocked(mockClient.customerPortal.subscriptions.list).mockResolvedValue(mockSubscriptions)
		...
		it('should list subscriptions by reference ID', async () => {
			...
			vi.mocked(mockClient.subscriptions.list).mockResolvedValue(mockSubscriptions)
		...
		it('should handle API errors for reference ID lookup', async () => {
			...
			await expect(handler(ctx)).rejects.toThrow('Subscriptions list with referenceId failed')
		})
		...
		it('should handle API errors for customer portal lookup', async () => {
			...
			await expect(handler(ctx)).rejects.toThrow('Polar subscriptions list failed')
		})
	})
	...
	describe('orders endpoint', () => {
		...
		it('should list customer orders with filters', async () => {
			...
			vi.mocked(mockClient.customerSessions.create).mockResolvedValue(mockSession)
			vi.mocked(mockClient.customerPortal.orders.list).mockResolvedValue(mockOrders)
