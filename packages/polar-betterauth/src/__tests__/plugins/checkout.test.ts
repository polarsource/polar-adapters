...
import { createMockPolarClient, createMockBetterAuthContext, createMockCheckout } from '../utils/mocks'
...
const { APIError, getSessionFromCtx } = await vi.importMock('better-auth/api') as any
	...
	describe('checkout endpoint handler', () => {
		...
		it('should create checkout with product IDs', async () => {
			...
			const ctx = {
				...
				json: vi.fn().mockReturnValue({ url: mockCheckout.url, redirect: true }),
			}
		...
		it('should handle async product resolution', async () => {
			const asyncProducts = vi.fn().mockResolvedValue([
		...
		it('should throw error for unauthenticated users when authentication required', async () => {
			...
			await expect(authHandler(ctx)).rejects.toThrow('You must be logged in to checkout')
		})
