...
import { createMockUser, createMockPolarClient } from './utils/mocks'
...
describe('polar plugin', () => {
	...
	beforeEach(() => {
		...
		vi.mocked(checkout).mockReturnValue(vi.fn().mockReturnValue({
			'checkout/create': vi.fn(),
		}))
		
		vi.mocked(portal).mockReturnValue(vi.fn().mockReturnValue({
			'portal/url': vi.fn(),
			'portal/customer': vi.fn(),
		}))
		
		vi.mocked(usage).mockReturnValue(vi.fn().mockReturnValue({
			'usage/ingest': vi.fn(),
		}))
		
		vi.mocked(webhooks).mockReturnValue(vi.fn().mockReturnValue({
	...
	it('should combine endpoints from all plugins', () => {
		const mockCheckoutPlugin = vi.fn().mockReturnValue({ 'checkout/create': vi.fn() })
